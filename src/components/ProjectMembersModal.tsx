...
import { X, User, Mail, Briefcase, Building, Calendar } from 'lucide-react';
          ...
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h2 className="text-xl font-semibold text-gray-900">
                Membres du projet "{projectName}"
              </h2>
              <p className="text-sm text-gray-500 mt-1">
                {members.length} membre{members.length > 1 ? 's' : ''} assigné{members.length > 1 ? 's' : ''} à ce projet
              </p>
            </div>
          </div>
          <button
          ...
          {members.length === 0 ? (
            <div className="text-center py-12">
              ...
              <p className="text-gray-500">
                Ce projet n'a actuellement aucun membre assigné à ses tâches.
              </p>
            </div>
          ) : (
                ...
                <div key={member.id} className="bg-gray-50 rounded-xl border border-gray-200 p-6 hover:shadow-md transition-all duration-200">
                  ...
                  <div className="space-y-3">
                    ...
                    <div className="flex items-center space-x-2 text-xs text-gray-500 pt-3 border-t">
                      ...
                      <span>Membre depuis le {member.created_at.toLocaleDateString('fr-FR')}</span>
                    </div>
                  </div>
                </div>
              ))}
